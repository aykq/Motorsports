...
2. **✅ TAMAMLANDI (2026-08-04/2026-08-06, commit `09f7029`) — Haber içeriği uyuşmazlığı.** Kök neden bulundu: `motorsportNews.ts`'teki `SHARE_PATTERN` regex'i kelime sınırı kullanmıyordu, "paylaş/kopyala/tweetle" gibi kökleri içeren gerçek editoryal paragrafları paylaşım-butonu metniymiş gibi yanlışlıkla siliyordu. Regex daraltıldı, backfill ile mevcut 101 kayıttan 15'i düzeltildi. **`redesign/vision`'da, main'e henüz merge/deploy edilmedi** (kullanıcı kararı: tam merge'e kadar bekleniyor).
...
4. **✅ TAMAMLANDI (2026-08-04, commit `b7d94bc`) — Em dash temizliği, uygulama geneli.** Kapsam 4 kategoriye ayrıldı, kullanıcı kararıyla sadece cümle-içi/i18n metin (C) ve DB'deki dinamik içerik (D — Gemini pist tarihçesi + haberler) temizlendi; sayfa başlığı ayıracı (A) ve boş-veri yer tutucusu (B) bilinçli olarak dokunulmadı. Gemini'nin sistem prompt'una "em dash kullanma" kuralı eklendi.
...
### Faz 10 — Ana sayfa F1-hub widget'ları ✅ TAMAMLANDI (2026-08-10, commit'ler `abc86e4..608992b`, subagent-driven-development ile, redesign/vision'a push'landı)

**Konu:** Non-F1 seri gizleme toggle'ı kapalıyken (varsayılan) ana sayfa tek F1 kartı + boş timeline gösteriyordu, kullanıcı "boş/sıkıcı" buldu. Spec: `docs/superpowers/specs/2026-08-10-home-f1-hub-widgets-design.md`, plan: `docs/superpowers/plans/2026-08-10-home-f1-hub-widgets.md`.

**Eklenenler:** `NextRaceHeroCard` + `StandingsTable` (`/f1` hub sayfasından paylaşılan bileşene çıkarıldı) + yeni `LastRacePodium` (son yarış podyumu) + `TopDriversStrip` (top-5 pilot, puan rozetli) + `DriverCard` (final review'da eklenen ortak pilot kartı — hem `/f1` hem ana sayfa kullanıyor) + `SeriesHubWidgets` (kompozisyon) + `src/lib/home-hub.ts` (saf yardımcı fonksiyonlar, vitest test edildi, `home-hub.test.ts`). Sadece tam 1 seri görünürken devreye giriyor, çoklu-seri modu hiç değişmedi.

**Final whole-branch review'da bulunan plan defect'i (düzeltildi):** `TopDriversStrip`'in ilk plan metni pilot fotoğraf kırpmasını (`objectPosition`) hardcode etmişti, F1'in kendi seri config değerini (`"center -5%"`) kullanmıyordu — `/f1`'in kendi pilot şeridi de ortak `DriverCard`'a taşınarak hem bug hem kod tekrarı çözüldü.

**Bekleyen (kozmetik, blocking değil):** Ana sayfadaki takım standings tablosunda `nameLabel` "İsim" diyor, "Takım" daha doğru olurdu.
