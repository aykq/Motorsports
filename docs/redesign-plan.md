...
**Sırada #1 — Pist tarihçesi/hikayesi. Tasarım onaylandı (2026-08-04), bkz. `docs/superpowers/specs/2026-08-04-circuit-history-design.md`.** Kaynak: Wikipedia (lede paragrafları) + f1-circuits.com (Circuit History bölümü), Gemini ile TR/EN özet (2-3 paragraf, ~200-300 kelime), mevcut "Pistler" sync butonuna entegre. Kaynak metin hash'lenip değişmediyse Gemini tekrar çağrılmaz (maliyet kontrolü). Kaynaklardaki YouTube/makale linkleri deterministik çıkarılıp özetin altında ayrı bir liste olarak gösterilir (LLM'e URL yazdırılmıyor, halüsinasyon riski). Implementasyon planı henüz yazılmadı.
