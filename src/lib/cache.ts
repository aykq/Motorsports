...
// No unstable_cache here: the list is a single indexed query and the page is
// already dynamic (auth cookies). Cross-request caching only bought staleness —
// revalidateTag() marks the entry stale without a synchronous refill, so the
// first reader after each 2-hourly scrape got served the stale list. React
// cache() still dedups repeat calls within one request (e.g. the detail page
// reads by id in both generateMetadata and the component).
export const getCachedNews = cache(async (slug: string, limit = 10): Promise<NewsItem[]> => {
  try {
...
export const getCachedNewsById = cache(async (id: string): Promise<NewsItem | null> => {
  try {
