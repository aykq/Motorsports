...
async function _findOpenF1Session(
  year: number,
  sessionDateISO: string,
  sessionType: string
): Promise<OpenF1Session | null> {
  const of1Name = SESSION_TYPE_TO_OF1_NAME[sessionType];
  if (!of1Name) return null;
  const sessions = await fetchOpenF1Sessions(year);
  const dayStr = new Date(sessionDateISO).toISOString().split("T")[0];
  return (
    sessions.find((s) => s.session_name === of1Name && s.date_start.startsWith(dayStr)) ?? null
  );
}

// Practice sessions have no "checkered flag" moment worth trusting as a results
// signal, and a fixed post-session delay is just a guess — some sessions publish
// lap data within minutes, others lag. Instead this waits for the lap-time results
// themselves to actually cover (nearly) every driver who was in the session, so the
// notification fires when there is something real to show rather than on a timer.
export async function openf1ArePracticeResultsComplete(
  year: number,
  sessionDateISO: string,
  sessionType: string,
  tolerance = 1
  ...
  try {
    const session = await _findOpenF1Session(year, sessionDateISO, sessionType);
    if (!session) return false;
    const [drivers, results] = await Promise.all([
      fetchOpenF1Drivers(session.session_key),
      fetchOpenF1PracticeResults(session.session_key),
    ]);
    if (!drivers.length) return false;
    return results.length >= Math.max(1, drivers.length - tolerance);
  } catch {
    return false;
  }
}
